 ...
 **Objective:** To compare different risk-based methods predicting individualized
 treatment effects in GRIT simulations. **Study Design and Setting:** We simulated
...
Combining all these settings resulted in a simulation study of 486 scenarios
...
adaptiveSelections <- readr::read_csv(here::here("data/processed/adaptiveModel.csv"))
adaptiveSelections <- adaptiveSelections %>% 
  group_by(scenarioId) %>% 
  nest() %>% 
  mutate(
...
The model including a constant relative treatment effect had the
lowest median RMSE in scenarios with a true constant relative
treatment effect (OR = 0.8, N = `r 4250` and AUC = 0.75) or with
moderate linear deviations (Supplement, Figure XX). However, when we
considered deviations from constant relative treatment effects, the
linear interaction model performed best most of the time, with RCS-3
having very comparable performance, outperforming the former in the
case of strong treatment-related harms and non-monotonic
deviations. The adaptive approach had comparable performance to the
best-performing method in each scenario, however, we observed
increased variability in the case of linear and non-monotonic
deviations, especially for moderate or strong treatment-related
harms. 

The adaptive approach selected the constant treatment effect model
`r adaptiveSelections %>% filter(scenarioId == 217) %>% pull(treatment) * 100`\% 
of the time, when no constant treatment-related harms were
considered, which dropped to
`r adaptiveSelections %>% filter(scenarioId == 219) %>% pull(treatment) * 100`\% 
in the case of strong treatment-related harms. When strong linear
deviations were considered, the adaptive approach favored the constant
treatment effect model
(`r adaptiveSelections %>% filter(scenarioId == 289) %>% pull(treatment) * 100`\%
of the time) in the absence of treatment-related harms, which was
replaced mainly by the linear interaction model (from
`r adaptiveSelections %>% filter(scenarioId == 289) %>% pull(risk) * 100`\%
to 
`r adaptiveSelections %>% filter(scenarioId == 291) %>% pull(risk) * 100`\%
of the time) with increasing treatment-related harms. In the case of
strong quadratic deviations, the adaptive approach selected the
linear interaction model the majority of the time which also increased
with stronger treatment-related harms (from
`r adaptiveSelections %>% filter(scenarioId == 361) %>% pull(risk) * 100`\%
with no harms to
`r adaptiveSelections %>% filter(scenarioId == 363) %>% pull(risk) * 100`\%
with strong harms ). RCS-3 selection frequency decreased with stronger
treatment-related harms (from
`r adaptiveSelections %>% filter(scenarioId == 361) %>% pull(rcs_3) * 100`\%
with no harms to
`r adaptiveSelections %>% filter(scenarioId == 363) %>% pull(rcs_3) * 100`\%
with strong harms ). Finally, with true non-monotonic deviations the
adaptive approach mainly selected the constant treatment effect model
(from
`r adaptiveSelections %>% filter(scenarioId == 397) %>% pull(treatment) * 100`\%
with no harms to
`r adaptiveSelections %>% filter(scenarioId == 399) %>% pull(treatment) * 100`\%
with strong harms), the linear interaction model (from
`r adaptiveSelections %>% filter(scenarioId == 397) %>% pull(risk) * 100`\%
with no harms to
`r adaptiveSelections %>% filter(scenarioId == 399) %>% pull(risk) * 100`\%
with strong harms) and the RCS-3 model
(from
`r adaptiveSelections %>% filter(scenarioId == 397) %>% pull(rcs_3) * 100`\%
with no harms to
`r adaptiveSelections %>% filter(scenarioId == 399) %>% pull(rcs_3) * 100`\%
with strong harms). For more details, see Supplement Table XX).
...
When we increased the sample size (N = `r 17000`), the model including
a constant relative treatment effect had the lowest RMSE under the
assumption of true constant relative treatment effects (Figure
\ref{fig:rmsesamplesize}; Panel A). However, when introducing strong
linear deviations the linear interaction model outperformed the
constant treatment effect model (Figure \ref{fig:rmsesamplesize};
Panel B). Furthermore, the linear interaction model was outperformed
by the more flexible RCS models (3 knots) in the case of strong
quadratic and non-monotonic deviations. Again, the increased
flexibility of RCS smoothing with higher number of knots resulted in
overfitting and worse predictive accuracy (Figure
\ref{fig:rmsesamplesize}; Panels C and D). The adaptive approach again
followed the performance of the best performing models in each
scenario. In addition, the increased variability in the adaptive
method's errors in the base scenario disappeared with large sample
size.
...
pronounced in the case of strong quadratic deviations. With increasing 
prediction AUC we saw an increased tendency of the adaptive approach to select
more complex models compared to the base case scenario. In the case of strong 
linear deviations, the linear interaction model was selected the majority of the
time (63\%), while the simpler constant treatment effect model was selected 26\%
of the time (Supplementary Table S7). However, the increased RMSE of the
constant treatment effect model inflated the the RMSE of the linear interaction
model. In the case of strong quadratic deviations the RCS (3 knots) model was
selected 84\% of the time.
...
Models including restricted cubic splines with 4 or 5 knots proved to be too
flexible, as indicated by higher RMSE, increased variability of discrimination
for benefit and worse calibration of benefit predictions. Even with larger
sample sizes and strong quadratic deviations from the base case scenario of
constant relative treatment effects, these more flexible restricted cubic
splines did not outperform the simpler RCS with 3 knots. These approaches may
only be helpful if we expect more extreme patterns of heterogeneous treatment
effects compared to the quadratic deviations considered here.
...
performance of the "true" model in most scenarios. However, with smaller sample
sizes it tended to "miss" the treatment-risk interactions and selected simpler
models (Supplementary Table S7). This resulted in increased RMSE variability in
these scenarios, especially in the case of true strong linear deviations from
the base case scenario. This tendency was mitigated when the discriminative
ability of the prediction model was higher. Therefore, in the case of smaller
sample sizes and/or poorly discriminating prediction models, the simpler linear
